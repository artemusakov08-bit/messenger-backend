...
class ChatController {
    ...
    async getUserChats(req, res) {
        try {
            // 🔥 ВАЖНО: используем правильное имя поля (userId, а не user_id)
            const userId = req.user.userId; 
            console.log('💬 Getting user chats for user:', userId);

            // Нормализуем ID (убираем "user_" для поиска)
            ...
            const messagesQuery = `
                ...
                FROM messages m
                WHERE m.chat_id LIKE $1 OR m.chat_id LIKE $2
                GROUP BY m.chat_id
                ORDER BY m.chat_id, MAX(m.timestamp) DESC
            `;
        
            // Ищем по обоим форматам (с префиксом и без)
            const messagesResult = await pool.query(messagesQuery, [
                `%${normalizedUserId}%`,  // ищем по числовому ID
                `%${userId}%`              // ищем по полному ID с префиксом
            ]);
            ...
            // 🔥 2. Также ищем чаты из таблицы chats напрямую
            ...
            const directChatsResult = await pool.query(directChatsQuery, [
                `%${normalizedUserId}%`,
                `%${userId}%`
            ]);
        
            // Добавляем чаты, которых еще нет в списке
            for (const chat of directChatsResult.rows) {
                const exists = allChats.some(c => c.id === chat.id);
                if (!exists) {
                    allChats.push({
                        id: chat.id,
